          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={formData.model} onValueChange={(v) => setFormData({ ...formData, model: v })}>
                <SelectTrigger className="focus:ring-primary"><SelectValue placeholder="Selecione um modelo" /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Input type="number" min="0" max="1" step="0.1" value={formData.temperature}
                onChange={(e) => setFormData({ ...formData, temperature: parseFloat(e.target.value) })} className="focus-visible:ring-primary" />
            </div>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Select value={formData.targetFunnelStage} onValueChange={(v) => setFormData({ ...formData, targetFunnelStage: v })}>
                <SelectTrigger className="focus:ring-primary"><SelectValue placeholder="Selecione o destino do lead" /></SelectTrigger>
                <SelectContent>
                  {destinations?.funnels?.map((funnel) => (
                    <SelectGroup key={funnel.id}>
                      <SelectLabel className="bg-muted">{funnel.name}</SelectLabel>
                      {funnel.stages.map((stage) => (
                        <SelectItem key={`stage:${stage.id}`} value={`stage:${stage.id}`}>{stage.name}</SelectItem>
                      ))}
                    </SelectGroup>
                  ))}
                  {destinations?.roulettes && destinations.roulettes.length > 0 && (
                    <SelectGroup>
                      ...
                      {destinations.roulettes.map((roulette) => (
                        <SelectItem key={`roulette:${roulette.id}`} value={`roulette:${roulette.id}`}>{roulette.name}</SelectItem>
                      ))}
                    </SelectGroup>
                  )}
                </SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Textarea rows={6} value={formData.systemPrompt}
                onChange={(e) => setFormData({ ...formData, systemPrompt: e.target.value })} className="focus-visible:ring-primary" />
        ...
        <Card className="transition-colors hover:border-primary/50">
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              <Textarea rows={6} value={formData.initiationStrategy}
                onChange={(e) => setFormData({ ...formData, initiationStrategy: e.target.value })}
                className="focus-visible:ring-primary"
                placeholder="Descreva como puxar assunto ou responder ao primeiro contato..." />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="transition-colors hover:border-primary/50">
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              <Textarea rows={6} value={formData.qualificationBoundary}
                onChange={(e) => setFormData({ ...formData, qualificationBoundary: e.target.value })}
                className="focus-visible:ring-primary"
                placeholder="Mapeie aqui o que você quer coletar. Ex: Nome, orçamento e bairro. Pare assim que coletar." />
            </div>
          </CardContent>
        </Card>

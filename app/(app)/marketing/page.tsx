...
import { FileText, Settings, Filter } from "lucide-react";
...
export default function MarketingPage() {
  ...
  const handleExportPDF = () => {
    // Aciona a caixa de impressão do navegador. 
    // O CSS se encarrega de ocultar o dashboard e mostrar o template.
    window.print();
  };

  // 1. LISTA DE OPÇÕES (Contas)
  const availableAccounts = useMemo(() => {
    if (!campaignData) return [];
    return Array.from(new Set(campaignData.map((d: any) => d.account_name).filter(Boolean))).sort() as string[];
  }, [campaignData]);

  // 2. LISTA DE OPÇÕES (Projetos/Campanhas)
  ...
  // 3. DADOS FILTRADOS (Campanhas)
  const filteredCampaigns = useMemo(() => {
    let filtered = campaignData || [];
    if (selectedAccount !== "all") {
      filtered = filtered.filter((d: any) => d.account_name === selectedAccount);
    }
  ...
  // 4. DADOS FILTRADOS (Criativos)
  const filteredCreatives = useMemo(() => {
    let filtered = creatives || [];
    if (selectedAccount !== "all") {
      filtered = filtered.filter((c: any) => c.account_name === selectedAccount);
    }
    return filtered;
  }, [creatives, selectedAccount]);

  // 5. DADOS FILTRADOS (Diários)
    ...
    if (source.length > 0 && source[0].account_name !== undefined) {
      if (selectedAccount !== "all") {
        ...
        if (selectedCampaign !== "all") {
          source = source.filter((m: any) => m.campaign_name === selectedCampaign);
        }
      }
  ...
  return (
    <>
      <div className="flex-1 space-y-4 p-8 pt-6 no-print">
        <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between space-y-4 xl:space-y-0">
          ...
          <div className="flex items-center space-x-2 flex-wrap gap-y-2">
            ...
            <Select value={selectedAccount} onValueChange={(val) => { setSelectedAccount(val); setSelectedCampaign("all"); }}>
            ...
            <Link href="/marketing/settings">
              <Button className="bg-secondary-custom text-primary-custom hover:bg-secondary-custom/90 transition-all shadow-custom-lg">
      ...
      {/* Template oculto que entra em cena exclusivamente na hora do window.print() */}

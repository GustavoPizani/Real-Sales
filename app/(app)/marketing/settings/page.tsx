...
import { Key, Eye, EyeOff, Save, Trash2, CheckCircle2, AlertCircle, Plus, ArrowLeft } from "lucide-react";
...
export default function MarketingSettingsPage() {
  ...
  const addAdAccount = () => { setAdAccounts([...adAccounts, ""]); };
  ...
  return (
    <div className="flex flex-col flex-1 w-full items-center justify-center bg-[#0a0a0a] min-h-[calc(100vh-4rem)] p-4 md:p-8">
      <div className="w-full max-w-2xl space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-secondary-custom">Configurações de Integração</h2>
          <Button 
            variant="outline" 
        ...
        <Card className="w-full bg-[#1A1A1A] border-[#333] text-white shadow-2xl">
        <CardHeader>
        ...
        <CardContent className="space-y-6">
          {API_KEYS.map((apiKey) => (
            <div key={apiKey.key} className="space-y-3 group">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium flex items-center gap-2 text-gray-300 group-hover:text-secondary-custom transition-colors">
                  {apiKey.label}
              ...
              <form className="flex gap-2" onSubmit={(e) => { e.preventDefault(); saveSetting(apiKey.key); }}>
                <div className="relative flex-1">
                  <Input
                    ...
                    autoComplete="off"
                    className="bg-black border-[#333] focus:border-secondary-custom pr-10 font-mono text-sm h-11 text-white"
                  />
                  <button type="button" onClick={() => setVisibility({ ...visibility, [apiKey.key]: !visibility[apiKey.key] })} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-white transition-colors">
                    {visibility[apiKey.key] ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </button>
                </div>
                <Button type="submit" variant="outline" size="icon" disabled={saving === apiKey.key} className="h-11 w-11 border-[#333] bg-[#1A1A1A] hover:bg-secondary-custom/10 hover:text-secondary-custom text-white">
                  <Save className="w-4 h-4" />
                </Button>
                {isConfigured(apiKey.key) && (
                  <Button type="button" variant="outline" size="icon" onClick={() => deleteSetting(apiKey.key)} disabled={saving === apiKey.key} className="h-11 w-11 border-[#333] bg-[#1A1A1A] hover:bg-red-500/10 hover:text-red-500 text-white">
          ...
          <div className="space-y-3 pt-6 border-t border-[#333] mt-6">
            <Label className="text-sm font-medium text-gray-300">Contas de Anúncio Meta (IDs)</Label>
            {adAccounts.map((account, index) => (
              <div key={index} className="flex gap-2">
                <Input 
                  ...
                  placeholder="act_..." 
                  className="bg-black border-[#333] focus:border-secondary-custom text-white h-11" 
                />
                {adAccounts.length > 1 && (
                  <Button variant="outline" size="icon" onClick={() => removeAdAccount(index)} className="h-11 w-11 border-[#333] bg-[#1A1A1A] hover:bg-red-500/10 hover:text-red-500 text-white">
                    <Trash2 className="w-4 h-4" />
                  </Button>
                )}
              </div>
            ...
            <div className="flex gap-2 mt-2">
                <Button variant="ghost" size="sm" onClick={addAdAccount} className="text-secondary-custom hover:text-secondary-custom/80 hover:bg-transparent">
